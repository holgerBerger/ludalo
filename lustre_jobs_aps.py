...
# first argument is passwd file to be able to map UIDs to usernames,
# following arguments are logfiles
...
# 2013-12-21 00:09:08: Released apid 3615838 resId 4072 pagg 0xba600000680
# claim


# FIXME does not yet contain handling of jobs not having start AND end in
# the single file
    ...
    for filename in sys.argv[2:]:
        ...
        for l in f:
            ...
            if "Placed apid" in l:
                ...
                sstart = sp[0] + " " + sp[1][:-1]
                start = int(
                    time.mktime(time.strptime(sstart, "%Y-%m-%d %H:%M:%S")))
                resid = getvalue(sp, "resId")
                uid = getvalue(sp, "uid")
            ...
            if "Released apid" in l:
                ...
                send = sp[0] + " " + sp[1][:-1]
                    ...
                    time.mktime(time.strptime(send, "%Y-%m-%d %H:%M:%S")))
                resid = getvalue(sp, "resId")
                try:
